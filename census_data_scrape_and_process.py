...
census_data = pd.read_html('https://en.wikipedia.org/wiki/List_of_U.S._states_by_historical_population',
                           match = '2010')
area_data = pd.read_html('https://en.wikipedia.org/wiki/List_of_U.S._states_and_territories_by_area',
                         match = 'Alaska')
...
census_data = census_data.merge(area_data,
                                how = 'left',
                                on = 'Name')

# Sort 'census_data' by 'Name'.
census_data.sort_values( by = 'Name',
                         inplace = True)
